...
void initGL() {
    ...
    pos.x=4;pos.y=1;pos.z=4;
    // pos.x=4;pos.y=0;pos.z=4;

    // l.x=-0.1;l.y=-0.08;l.z=-0.1;
    l.x=-0.1;l.y=-0.025;l.z=-0.1;
    // l.x=-0.1;l.y=0;l.z=-0.1;
    u.x=0;u.y=0.1;u.z=0;
    r.x=0.1;r.y=0;r.z=0;
...
void drawGrid(){
    ...
    glLineWidth(3);
    const int mx = 100;
    const int segment = mx;
    glBegin(GL_LINES);{
        glVertex3d(0, -sphereRadius, mx);
        glVertex3d(0, -sphereRadius, -mx);
        glVertex3d(-mx, -sphereRadius, 0);
        glVertex3d(mx, -sphereRadius, 0);
    }glEnd();
    ...
    glBegin(GL_LINES);{
        for(int i = -segment; i <= segment; i++){
            glVertex3d(1.0 * mx * i / segment, -sphereRadius, mx);
            glVertex3d(1.0 * mx * i / segment, -sphereRadius, -mx);
            glVertex3d(-mx, -sphereRadius, 1.0 * mx * i / segment);
            glVertex3d(mx, -sphereRadius, 1.0 * mx * i / segment);
        }
    }glEnd();
}
...
void display() {
    ...
    drawGrid();    
    GLdouble dis = sphereRadius * angleSide * M_PI / 180;
    glTranslated(0, 0, dis);
    glRotated(angleSide, 1, 0, 0);
