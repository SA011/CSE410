...
bool isInsideSquare(Point p1, Point p2, Point p3, Point p4, Point p){
    if(!isInsideTriangle(p1, p2, p3, p) && !isInsideTriangle(p1, p3, p4, p))return false;
    return true;   
}
...
struct Sphere : public Object{
    ...
    Point getNormal(Line ray, Point p){
        Point norm = unit(p-center);
        if(dot(norm, ray.v) > 0)norm = -norm;
        return norm;
    }
};
...
struct Pyramid : public Object{
    ...
    Point getNormal(Line ray, Point p){
        ...
        Point ret = {0, 1, 0};
        bool ok = false;
        if(!ok && isInsideTriangle(base1, base2, up, p))ret = unit(cross(base2 - base1, up - base1)), ok = true;
        if(!ok && isInsideTriangle(base2, base3, up, p))ret = unit(cross(base3 - base2, up - base2)), ok = true;
        if(!ok && isInsideTriangle(base3, base4, up, p))ret = unit(cross(base4 - base3, up - base3)), ok = true;
        if(!ok && isInsideTriangle(base4, base1, up, p))ret = unit(cross(base1 - base4, up - base4)), ok = true;

        if(!ok && isInsideSquare(base1, base2, base3, base4, p))ret = unit(p - up), ok = true;        

        if(dot(ret, ray.v) > 0)ret = -ret;
        return ret;
    }
...
struct Cube : public Object{
    ...
    Point getNormal(Line ray, Point p){
        ...
        bool ok = false;
        Point ret = {0, 1, 0};
        if(!ok && isInsideSquare(p1, p2, p3, p4, p))ret = unit(Point(0, -1, 0)), ok = true;
        if(!ok && isInsideSquare(p1, p2, p6, p5, p))ret = unit(Point(0, 0, -1)), ok = true;
        if(!ok && isInsideSquare(p2, p3, p7, p6, p))ret = unit(Point(1, 0, 0)), ok = true;
        if(!ok && isInsideSquare(p3, p4, p8, p7, p))ret = unit(Point(0, 0, 1)), ok = true;
        if(!ok && isInsideSquare(p4, p1, p5, p8, p))ret = unit(Point(-1, 0, 0)), ok = true;
        if(!ok && isInsideSquare(p5, p6, p7, p8, p))ret = unit(Point(0, 1, 0)), ok = true;
        assert(ok);
        if(dot(ret, ray.v) > 0)ret = -ret;
        return ret;
    }
...
struct CheckerBoard : public Object{
    ...
    Color getColor(Point p){
        int x = floor(p.x / sizeOfCell);
        int z = floor(p.z / sizeOfCell);
        if((x+z)%2==0){
            if(Data::isTexture == false) return {0,0,0};
            else{
                int x1 = (p.x - sizeOfCell * x) / sizeOfCell * Data::texture_b.width();
                int z1 = (p.z - sizeOfCell * z) / sizeOfCell * Data::texture_b.height();
                if(x1 < 0)x1 += Data::texture_b.width();
                if(z1 < 0)z1 += Data::texture_b.height();
                // cout << x1 << ' ' << z1 << endl;
                assert(z1 >= 0 && z1 < Data::texture_b.height());
                assert(x1 >= 0 && x1 < Data::texture_b.width());
                unsigned char r, g, b;
                Data::texture_b.get_pixel(x1, z1, r, g, b);
                //  cout << (int)r << " " << (int)g << " " << (int)b << endl;
                return {r/255.0, g/255.0, b/255.0};
            }
        }
        if(Data::isTexture == false)return {1, 1, 1};
        else{
            int x1 = (p.x - sizeOfCell * x) / sizeOfCell * Data::texture_w.width();
            int z1 = (p.z - sizeOfCell * z) / sizeOfCell * Data::texture_w.height();
            if(x1 < 0)x1 += Data::texture_w.width();
            if(z1 < 0)z1 += Data::texture_w.height();
            unsigned char r, g, b;
                // cout << x1 << ' ' << z1 << endl;
                assert(x1 >= 0 && x1 < Data::texture_w.width());
                assert(z1 >= 0 && z1 < Data::texture_w.height());
            Data::texture_w.get_pixel(x1, z1, r, g, b);
            // cout << (int)r << " " << (int)g << " " << (int)b << endl;
            return {r/255.0, g/255.0, b/255.0};
        }
    }
...
void inputData(string filename, string tex1, string tex2){
    ...
    Data::r = Point(0,0,1);
    Data::u = Point(0,1,0);
    Data::l = cross(Data::u, Data::r);
    Data::D = length(Data::pos);

    Data::texture_b = bitmap_image(tex1);
    Data::texture_w = bitmap_image(tex2);
...
void clear(){
    for(auto x : Data::objects)delete x;
    for(auto x : Data::lights)delete x;
    for(auto x : Data::spotLights)delete x;
...
int main(int argc, char** argv) {
    ...
    inputData("description.txt", "texture_b.bmp", "texture_w.bmp");
